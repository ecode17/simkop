...
<section style="background: #f7fafc; margin-top: 50px;">
    <div class="container py-5">
        <div class="row mb-4">
            <div class="col text-center">
                <h2 class="fw-bold" style="color:#1e6d58;letter-spacing:-.5px">Berita & Artikel Terbaru</h2>
                <p class="text-muted mb-0 fs-5" style="color:#43766C">
                    Dapatkan informasi dan update seputar koperasi, simpan pinjam, serta tips keuangan terkini.
                </p>
            </div>
        </div>
        <div class="row g-4">
            <?php foreach ($news as $item):
                // Slug SEO dari judul sesuai permintaan (gunakan tag untuk nama provinsi)
                $namaWilayah = $item['tag'] ?? 'provinsi';
                $slug = url_title('jasa pembuatan aplikasi koperasi di ' . $namaWilayah, '_', true);
            ?>
                <div class="col-12 col-md-6 col-lg-4 d-flex align-items-stretch">
                    <div class="card shadow-sm border-0 rounded-4 w-100 h-100 hover-shadow"
                        ...
                        <div class="card-body px-4 py-3 d-flex flex-column">
                            ...
                            <a href="<?= site_url('aplikasi/' . $item['id_news'] . '/' . $slug) ?>" class="btn btn-outline-success btn-sm rounded-pill mt-2 align-self-start px-3">
                                Baca Selengkapnya
                            </a>
                        </div>
                    </div>
        ...
        <?php
        $totalPages = $pager->getPageCount('news');
        $currentPage = $pager->getCurrentPage('news');
        $range = 1; // tampilkan halaman +/-1 dari current
        $showFirstLast = $totalPages > 5;
        function page_link($page, $pager, $label = null, $active = false)
        {
            $url = $pager->getPageURI($page, 'news');
                    ...
                    <nav aria-label="Page navigation">
                        <ul class="pagination pagination-slim shadow-sm mb-0">
                            ...
                            <li class="page-item<?= ($currentPage == 1 ? ' disabled' : '') ?>">
                                <a class="page-link" href="<?= ($currentPage == 1) ? '#' : $pager->getPageURI($currentPage - 1, 'news') ?>" aria-label="Previous">
                            ...
                            <?php if ($showFirstLast && $currentPage > $range + 2): ?>
                                <?= page_link(1, $pager, '1', $currentPage == 1) ?>
                            ...
                            for ($i = $start; $i <= $end; $i++):
                                ...
                                echo page_link($i, $pager, null, $currentPage == $i);
                            endfor;
                            ?>
                            ...
                            <?php if ($showFirstLast && $currentPage < $totalPages - ($range + 1)): ?>
                                <li class="page-item disabled"><span class="page-link">...</span></li>
                                <?= page_link($totalPages, $pager, $totalPages, $currentPage == $totalPages) ?>
                            <?php endif; ?>
                            ...
                            <li class="page-item<?= ($currentPage == $totalPages ? ' disabled' : '') ?>">
                                <a class="page-link" href="<?= ($currentPage == $totalPages) ? '#' : $pager->getPageURI($currentPage + 1, 'news') ?>" aria-label="Next">
